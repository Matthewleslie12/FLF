  ...
  return (
    <div className="m-lg ">
      ...
      <div className="w-full md:w-[17rem] h-full lg:max-w-[20rem]">
        ...
        <div className="hidden h-[32.8125rem] lg:flex flex-col  max-w-[17rem]">
            
         {FiendBios.bios.map((fiend)=>{
          if (fiend.id === activeFiend){
            return <div className="flex first:self-end second:self-center second:mr-[1rem] fourth:self-center fourth:mr-[1rem] last:self-end"><FiendPhotoCard pic={fiend.id} isActive={true} setActiveFiend={setActiveFiend}/></div>
          }else{
            return <div className="flex first:self-end second:self-center second:mr-[1rem] fourth:self-center fourth:mr-[1rem] last:self-end"><FiendPhotoCard pic={fiend.id} isActive={false} setActiveFiend={setActiveFiend}/></div>
          }
        })}
